...
async function main() {
  program
    ...
    .action(async () => {
      ...
      if (answers.timelock) {
        stdout.printStep(
          `Queuing set pending timelock admin to ${answers.admin} on eta ${answers.eta}`
        );
        const txHash = await timelocked_setPendingTimelockAdmin(
          answers.admin,
          "queue",
          answers.eta
        );
  ...
  program
    ...
    .action(async () => {
      await printNetwork();
      const { delay, maximumDelay, minimumDelay, gracePeriod } =
        await getTimelockSettings();
  ...
  program
    ...
    .action(async () => {
      ...
      stdout.printStep(
        `Executing transaction with id ${answers.transactionId}`
      );
