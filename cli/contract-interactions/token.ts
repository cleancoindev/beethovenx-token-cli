...
export async function printCirculatingSupply() {
  ...
  const teamVesting = await token.balanceOf(
    config.contractAddresses.TeamVesting
  );
  ...
  stdout.printInfo(
    `Vested team: ${ethers.utils.formatUnits(
      teamVesting,
      await token.decimals()
    )}`
  );
  stdout.printInfo(
    `Treasury: ${ethers.utils.formatUnits(treasury, await token.decimals())}`
  );
  stdout.printInfo(
    `Partnership: ${ethers.utils.formatUnits(
      partnership,
      await token.decimals()
    )}`
  );
  stdout.printInfo(
    `Team: ${ethers.utils.formatUnits(team, await token.decimals())}`
  );
